...
class HomeController extends Controller
{
    ...
    public function getLoaiSanPham()
    {
        ...
        // Truyền danh sách loại sản phẩm vào view frontend.sach.phanloai
        return view('frontend.sach.phanloai', ['dsLoaiSanPham' => $dsLoaiSanPham]);
    }

	public function getSanPham($tenloai_slug = '')
	{
        // Lấy thông tin loại sản phẩm dựa trên tenloai_slug
        $loaisach = LoaiSach::where('tenloai_slug', $tenloai_slug)->first();
        ...
        $dssp = SanPham::where('loaisanpham_id', $loaisach->id)->get();

        // Truyền thông tin loại sản phẩm và danh sách sản phẩm vào view bằng compact
        return view('frontend.sanpham', compact('loaisach', 'dssp'));
	}

	public function getSanPham_ChiTiet($tenloai_slug = '', $tensanpham_slug = '')
	{
        // Tìm thông tin sản phẩm dựa trên các tham số nhận được
        $sanpham = SanPham::join('loaisach', 'sach.loaisanpham_id', '=', 'loaisach.id')
            ->where('loaisach.tenloai_slug', $tenloai_slug)
            ->where('sach.tensanpham_slug', $tensanpham_slug)
            ->first();
...
// Sau đó đăng nhập
	...
	/*public function getDatHangDemo()
	{
		...
		$donhang_chitiet = new _ChiTietDonHang();
